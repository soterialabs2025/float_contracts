...
contract AutoStrategyV2 is AutoStrategyManagerV2, ReentrancyGuard, IERC721Receiver, IAutoStrategy {
    ...
    /// @dev Drain LP, fund 50/50 deficit from reserve then swap, mint deployable only. Does not refill reserve.
    ...
    /// @dev Pull the short side from reserve toward a 50/50 of current deployable value.
    function _fundDeficitFromReserve(uint256 assetBal, uint256 wethBal) internal {
        ...
        uint256 targetAsset = Math.mulDiv(totalValue, TARGET_ASSET_BPS, DIVISOR);
    ...
    /// @dev After 50/50 balance of deployable, credit `reserveBps` of each leg into reserved buckets.
