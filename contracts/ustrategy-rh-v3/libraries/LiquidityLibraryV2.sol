...
library LiquidityLibraryV2 {
    ...
    function getSqrtRatios(int24 lowerTick, int24 upperTick) public pure returns (uint160 sqrtL, uint160 sqrtU) {
        sqrtL = TickMath.getSqrtRatioAtTick(lowerTick);
        sqrtU = TickMath.getSqrtRatioAtTick(upperTick);
    }

    function calculateMinAmounts(uint256 amount0, uint256 amount1, uint16 slippageBps) public
        pure
    ...
    function getPositionLiquidity(PositionState storage ps, INonfungiblePositionManager npm) public
        view
        returns (uint128 liquidity)
    {
    ...
    function decreaseAllLiquidity(PositionState storage ps, DecreaseContext memory ctx) public
    ...
    function decreaseLiquidityByAmount(PositionState storage ps, DecreaseContext memory ctx, uint128 liqToRemove) public
