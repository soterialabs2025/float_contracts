...
contract AutoKeeper is IAutoKeeper, Ownable, ReentrancyGuard {
    ...
    IAutoOperatorRegistry public immutable operatorRegistry;
    ...
    function addStrategy(address strat) external override onlyStrategyFactory returns (uint256 id) {
        ...
        id = watched.length - 1;
        IAutoStrategyV3(strat).setWatched(true);
    ...
    function _performUpkeep(uint256 id) internal {
        ...
        IAutoStrategyV3 strat = IAutoStrategyV3(ws.stratAddr);
    ...
    function _recordVaultPoolValueSnapshot(address stratAddr) internal {
        address vaultAddr = IAutoStrategyV3(stratAddr).vault();
