...
/// @notice Per-vault LiquidSharesBv3 staking with fixed-length epochs.
...
contract ShareStakingBv3 is Ownable, ReentrancyGuard, IShareStakingBv3 {
    ...
    ILiquidSharesBv3 public liquidShares;
    ...
    function bootstrap(
        ...
        liquidShares = ILiquidSharesBv3(liquidShares_);
        strategy = strategy_;
        asset = asset_;
    ...
    /// @notice Withdraw LiquidSharesBv3. Reverts until `lockedUntil` (end of epoch in which stake was acquired).
    ...
    function _swapWethToAsset(uint128 amount) internal returns (uint256 assetAdded) {
        uint256 minOut = IAutoStrategyBv3(strategy).minOutForSwap(address(weth), amount);
