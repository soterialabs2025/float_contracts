...
/// @title AutoKeeperRhV3
/// @notice Upkeep = remint path only (`keeperCheck`); harvest is a separate cadence.
contract AutoKeeperRhV3 is IAutoKeeperRhV3, Ownable, ReentrancyGuard {
    ...
    IAutoOperatorRegistryRhV3 public immutable operatorRegistry;
    ...
    function addStrategy(address strat) external override onlyStrategyFactory returns (uint256 id) {
        ...
        id = watched.length - 1;
        IAutoStrategyRhV3(strat).setWatched(true);
    ...
    function _performUpkeep(uint256 id) internal {
        ...
        IAutoStrategyRhV3 strat = IAutoStrategyRhV3(ws.stratAddr);
    ...
    function _recordVaultPoolValueSnapshot(address stratAddr) internal {
        address vaultAddr = IAutoStrategyRhV3(stratAddr).vault();
        if (vaultAddr == address(0)) revert ZeroAddress();
        IAutoVaultRhV3(vaultAddr).recordPoolValueSnapshot();
    }
}
