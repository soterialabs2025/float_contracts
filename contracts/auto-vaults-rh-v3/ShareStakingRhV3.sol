...
import "./interfaces/ILiquidSharesRhV3.sol";

/// @title ShareStakingRhV3
/// @notice Per-vault LiquidSharesRhV3 staking with fixed-length epochs.
...
contract ShareStakingRhV3 is Ownable, ReentrancyGuard, IShareStakingRhV3 {
    ...
    ILiquidSharesRhV3 public liquidShares;
    ...
    /// @dev `assetAdded == 0` means WETH→ASSET soft-fail (tokens stranded for rescue/retry).
    ...
    function bootstrap(
        ...
        liquidShares = ILiquidSharesRhV3(liquidShares_);
        strategy = strategy_;
        asset = asset_;
    ...
    /// @notice Set owner cut of epoch ASSET rewards. Cap 10%. Only after package ownership transfer.
    ...
    function _takeOwnerEpochCut(uint256 epoch) internal returns (uint256 ownerCut) {
        ...
        // accountedAsset unchanged: liability moves from epoch pot to pendingOwnerReward.
    }
    ...
    /// @notice Withdraw LiquidSharesRhV3. Reverts until `lockedUntil` (end of epoch in which stake was acquired).
    ...
    /// @inheritdoc IShareStakingRhV3
    /// @dev Strategy must transfer `amount` of `token` to this contract before calling.
    ///      WETH→ASSET swap failures are soft: tokens stay here for rescue/retry; harvest does not revert.
    ...
    function _swapWethToAsset(uint128 amount) internal returns (uint256 assetAdded) {
        uint256 minOut = IAutoStrategyRhV3(strategy).minOutForSwap(address(weth), amount);
    ...
    /// @notice Pull owner-cut ASSET credited at epoch finalize.
    ...
    function retryWethRewardSwap(uint256 amount) external nonReentrant onlyOwner {
        ...
        if (amount > type(uint128).max) revert ZeroAmount();
        _advanceGlobalTo(block.timestamp);
        uint256 minOut = IAutoStrategyRhV3(strategy).minOutForWithdraw(address(weth), amount);
