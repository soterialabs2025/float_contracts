...
import "./interfaces/IAutoStrategyV3Rh.sol";
...
contract AutoVaultV3Rh is Ownable, ReentrancyGuard, Pausable, IAutoVaultV2 {
    using SafeERC20 for IERC20;

    IWETHV3Rh public immutable weth;
    IAutoStrategyV3Rh public strategy;
    ...
    function bootstrap(address owner_, address strategy_, address liquidToken_, address asset_) external {
        ...
        factory = msg.sender;
        strategy = IAutoStrategyV3Rh(strategy_);
    ...
    function depositWeth(uint256 amount) external override nonReentrant whenNotPaused returns (uint256 shares) {
        ...
        return _mintSharesAndDeploy(amount);
    }
    ...
    function withdraw(uint256 shares, bool asAsset) external override nonReentrant whenNotPaused returns (uint256) {
        ...
        IAutoStrategyV3Rh.WithdrawToken out =
            asAsset ? IAutoStrategyV3Rh.WithdrawToken.ASSET : IAutoStrategyV3Rh.WithdrawToken.WETH;
