...
contract AutoFactoryV3Rh is Ownable {
        ...
        returns (address strategy, address vault, address liquidShares, address shareStaking, uint256 keeperId)
    {
        ...
        AutoVaultV3Rh(payable(vault)).bootstrap(msg.sender, strategy, liquidShares, shareStaking, asset);
        AutoStrategyV3Rh(payable(strategy)).bootstrap(
            msg.sender,
            vault,
            infra.swapRouter,
    ...
    function transferPackageOwnership(address asset, address newOwner) external {
        ...
        if (
            AutoVaultV3Rh(payable(pkg.vault)).ownershipLocked()
                || AutoStrategyV3Rh(payable(pkg.strategy)).ownershipLocked()
                || ShareStaking(pkg.shareStaking).ownershipLocked()
        ) revert PackageOwnershipLocked();

        address vaultOwner = AutoVaultV3Rh(payable(pkg.vault)).owner();
        address strategyOwner = AutoStrategyV3Rh(payable(pkg.strategy)).owner();
        ...
        AutoVaultV3Rh(payable(pkg.vault)).transferOwnershipFromFactory(newOwner);
        AutoStrategyV3Rh(payable(pkg.strategy)).transferOwnershipFromFactory(newOwner);
