...
contract AutoStrategySv3 is AutoStrategyManagerSv3, ReentrancyGuard, IERC721Receiver, IAutoStrategySv3 {
    ...
    function setSwapRouter(address router_) external onlyOwner {
        ...
        swapRouter = IAutoSwapRouterSv3(router_);
    }
    ...
    /// @dev Split protocol fee: stakingShareBps → ShareStakingSv3, rest → feeManager.
    ...
    function _bandShare(int24 lower, int24 upper, int24 tick) internal view returns (uint256) {
        return LiquidityLibraryV2.mintShare(lower, upper, tick, _pool.token0() != address(WETH));
    }
