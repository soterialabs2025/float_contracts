...
import "./AutoStrategyManagerSv3.sol";
...
contract AutoStrategySv3 is AutoStrategyManagerSv3, ReentrancyGuard, IERC721Receiver, IAutoStrategySv3 {
    ...
    IAutoOperatorRegistrySv3 public operatorRegistry;
    ...
    function bootstrap(
        ...
        if (_bootstrapped || msg.sender != factory) revert E();
        if (
    ...
    /// @dev Split protocol fee token amount: stakingShareBps → ShareStakingSv3 (as epoch rewards), rest → feeManager.
    ///      ShareStakingSv3 soft-fails ASSET→WETH swaps; try/catch here still protects harvest if notify reverts.
    ...
    function withdraw(uint256 userShares, address receiver, WithdrawToken outToken) external override nonReentrant {
        ...
        IAutoVaultSv3 v = IAutoVaultSv3(vault);
