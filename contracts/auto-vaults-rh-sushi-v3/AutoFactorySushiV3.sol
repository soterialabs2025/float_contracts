...
import "./AutoStrategySushiV3.sol";
import "./AutoVaultSushiV3.sol";
import "./LiquidShares.sol";
import "./ShareStakingSushi.sol";
...
contract AutoFactorySushiV3 is Ownable {
    ...
    constructor(InfraConfig memory config) Ownable(msg.sender) {
        ...
        infra = config;
        strategyImplementation = address(new AutoStrategySushiV3(address(this)));
        vaultImplementation = address(new AutoVaultSushiV3());
        liquidSharesImplementation = address(new LiquidShares());
        shareStakingImplementation = address(new ShareStakingSushi());
    }
        ...
        returns (address strategy, address vault, address liquidShares, address shareStaking, uint256 keeperId)
    {
        ...
        ShareStakingSushi(shareStaking).bootstrap(
            msg.sender, liquidShares, strategy, asset, infra.swapRouter, poolFee
        );
        AutoVaultSushiV3(payable(vault)).bootstrap(msg.sender, strategy, liquidShares, shareStaking, asset);
        AutoStrategySushiV3(payable(strategy)).bootstrap(
            msg.sender,
            vault,
            infra.swapRouter,
        ...
        // ShareStakingSushi swaps ASSET→WETH for epoch rewards via the same router whitelist.
    ...
    /// @notice One-shot transfer of package admin ownership (vault + strategy + ShareStakingSushi) to `newOwner`.
    ...
    function transferPackageOwnership(address asset, address newOwner) external {
        ...
        if (
            AutoVaultSushiV3(payable(pkg.vault)).ownershipLocked()
                || AutoStrategySushiV3(payable(pkg.strategy)).ownershipLocked()
                || ShareStakingSushi(pkg.shareStaking).ownershipLocked()
        ) revert PackageOwnershipLocked();

        address vaultOwner = AutoVaultSushiV3(payable(pkg.vault)).owner();
        address strategyOwner = AutoStrategySushiV3(payable(pkg.strategy)).owner();
        address stakingOwner = ShareStakingSushi(pkg.shareStaking).owner();
        ...
        AutoVaultSushiV3(payable(pkg.vault)).transferOwnershipFromFactory(newOwner);
        AutoStrategySushiV3(payable(pkg.strategy)).transferOwnershipFromFactory(newOwner);
        ShareStakingSushi(pkg.shareStaking).transferOwnershipFromFactory(newOwner);
