...
contract SoteriaFeeManagerSv3 is Ownable, ReentrancyGuard {
    ...
    /// @param deadline Unix timestamp after which the swap reverts. Must be non-zero.
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut = v3SwapRouter.swapExactInputSingleStrict(
            token, address(WETH), fee, uint128(amountIn), minOut, deadline
        );
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut = v3SwapRouter.swapExactInputSingleStrict(
            address(WETH), address(usdc), fee, uint128(amountIn), minOut, deadline
        );
