...
contract AutoKeeperSv3 is IAutoKeeperSv3, Ownable, ReentrancyGuard {
    ...
    IAutoOperatorRegistrySv3 public immutable operatorRegistry;
    ...
    function addStrategy(address strat) external override onlyStrategyFactory returns (uint256 id) {
        ...
        id = watched.length - 1;
        IAutoStrategySv3(strat).setWatched(true);
    ...
    function _performUpkeep(uint256 id) internal {
        ...
        IAutoStrategySv3 strat = IAutoStrategySv3(ws.stratAddr);
    ...
    function _recordVaultPoolValueSnapshot(address stratAddr) internal {
        address vaultAddr = IAutoStrategySv3(stratAddr).vault();
        if (vaultAddr == address(0)) revert ZeroAddress();
        IAutoVaultSv3(vaultAddr).recordPoolValueSnapshot();
    }
}
