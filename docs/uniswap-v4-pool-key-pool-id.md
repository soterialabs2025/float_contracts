...
Source: [Cyfrin Updraft — PoolManager: PoolKey Example](https://updraft.cyfrin.io/courses/uniswap-v4/pool-manager/pool-key-example)
...
> **Note (v4-core types):** In `@uniswap/v4-core`, `currency0` / `currency1` are `Currency` UDVTs, so production code typically uses `Currency.wrap(address(0))`, `Currency.wrap(WBTC)`, and `IHooks(address(0))` after imports.
